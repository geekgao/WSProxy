...
if [ "${pem_output}" = true ]; then
    echo "------------- convert to pem format -----------------------"
    openssl pkcs12 -in ${srv_p12_file} -out ${sdir}/${srv_key_name}.pem -nodes
    echo "PEM file generated at ${sdir}/${srv_key_name}.pem"
fi